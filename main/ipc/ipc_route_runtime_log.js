...
function create_ipc_route_runtime_log(deps) {
  ...
  return [
    create_ipc_route_spec(
      IPC_CHANNELS.RUNTIME_LOG_STATUS,
      IPC_ROUTE_AUTH.OPTIONAL,
      () => runtime_log_service.get_runtime_log_status()
    ),
    create_ipc_route_spec(
      IPC_CHANNELS.RUNTIME_LOG_SET_ENABLED,
      IPC_ROUTE_AUTH.OPTIONAL,
      (enabled) => runtime_log_service.set_runtime_logs_enabled(enabled)
    ),
    create_ipc_route_spec(
      IPC_CHANNELS.RUNTIME_LOG_OPEN_CONSOLE,
      IPC_ROUTE_AUTH.OPTIONAL,
      () => {
    ...
    create_ipc_route_spec(
      IPC_CHANNELS.RUNTIME_LOG_APPEND,
      IPC_ROUTE_AUTH.OPTIONAL,
      (payload) => runtime_log_service.append_runtime_log(payload)
    ),
    create_ipc_route_spec(
      IPC_CHANNELS.RUNTIME_LOG_LOAD,
      IPC_ROUTE_AUTH.OPTIONAL,
      () => ({
        enabled: runtime_log_service.is_runtime_logs_enabled(),
