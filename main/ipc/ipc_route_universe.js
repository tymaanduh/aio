...
function create_ipc_route_universe(deps) {
  ...
  return [
    create_ipc_route_spec(
      IPC_CHANNELS.UNIVERSE_LOAD_CACHE,
      IPC_ROUTE_AUTH.REQUIRED,
      () => repository_universe.load_universe_cache_state()
    ),
    create_ipc_route_spec(
      IPC_CHANNELS.UNIVERSE_SAVE_CACHE,
      IPC_ROUTE_AUTH.REQUIRED,
      (payload) => repository_universe.save_universe_cache_state(payload)
    ),
    create_ipc_route_spec(
      IPC_CHANNELS.UNIVERSE_EXPORT,
      IPC_ROUTE_AUTH.REQUIRED,
      (payload) => repository_universe.export_universe(payload)
    )
  ];
}
