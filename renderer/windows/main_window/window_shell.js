...
const WINDOW_PRE_LOAD_HOOK_ENTRY = Object.freeze({
  ...
  connectors_before: PATH_LIST([
    [T.RENDERER, T.VIEWS, T.MAIN_WINDOW, "html"],
    [T.RENDERER, T.WINDOWS, T.MAIN_WINDOW, T.WINDOW, T.SHELL]
  ]),
  data_structures: TXT_LIST(PATH(T.WINDOW, T.DOCUMENT), PATH(T.WINDOW, T.PAGE), PATH(T.WINDOW, T.APP_API)),
  argument_specs: [
    {
      name: T.WINDOW_SCOPE,
      type: "string",
...
const WINDOW_POST_LOAD_HOOK_ENTRY = Object.freeze({
  ...
  linked_objects_after: PATH_LIST([
    [T.WINDOW, T.PAGE, T.HOOK, T.GET_TREE],
    [T.WINDOW, T.PAGE, T.HOOK, T.GET_ARGUMENTS],
    [T.WINDOW, T.APP_LOAD_HOOK, T.SEARCH]
  ]),
  data_structures: TXT_LIST(T.LOAD_HOOK_REGISTRY, T.HOOK_TRACE),
  tags: TAG_LIST(T.WINDOW, T.MAIN_WINDOW, T.POST_LOAD)
});

async function run_main_window_shell() {
  let ctx = {
    window_scope: T.MAIN_WINDOW,
    hook_trace: []
  };
...
run_main_window_shell().catch((error) => {
  // eslint-disable-next-line no-console
  console.error("main_window shell bootstrap failed", error);
});
