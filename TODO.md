...
- [ ] Keep all agent ids synchronized across:
  - `to-do/agents/*.yaml`
  - `to-do/agents/agents_registry.yaml`
  - `to-do/agents/agent_access_control.json`
  - `to-do/skills/agent_workflows.json`
- [ ] Enforce startup tool caps and non-startup tool access request logging.
- [ ] Enforce full update logging (scan at start/end, watch during long edit sessions).
- [ ] Keep all skill agent metadata on the `interface` + `policy` schema with `allow_implicit_invocation: true` unless explicitly overridden.
- [ ] Run `npm run agents:validate` before workflow completion.
