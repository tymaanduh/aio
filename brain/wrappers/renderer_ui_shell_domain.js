...
(function (root, factory) {
  ...
  function setStatus(message, isError = false) {
    ...
    nextKey !== G_RT.lastStatusLog &&
      ((G_RT.lastStatusLog = nextKey), pushRuntimeLog(isError ? "error" : "info", "status", message, "saveStatus"));
  ...
  function syncExplorerLayoutControls() {
    ...
    G_DOM.explorerCompactAction instanceof HTMLElement &&
      (G_DOM.explorerCompactAction.classList.toggle("active", compactActive),
      G_DOM.explorerCompactAction.setAttribute("aria-pressed", compactActive ? "true" : "false"));
    G_DOM.explorerFocusAction instanceof HTMLElement &&
      (G_DOM.explorerFocusAction.classList.toggle("active", focusActive),
      G_DOM.explorerFocusAction.setAttribute("aria-pressed", focusActive ? "true" : "false"));
  }
  ...
  function setExplorerLayoutMode(mode, options = {}) {
    ...
    G_DOM.appRoot instanceof HTMLElement &&
      (G_DOM.appRoot.classList.toggle("explorer-compact", normalized === EXPLORER_LAYOUT_COMPACT),
      G_DOM.appRoot.classList.toggle("explorer-maximized", normalized === EXPLORER_LAYOUT_MAXIMIZED));
  ...
  function setTreeFolderSelection(groupKey, label = "", options = {}) {
    ...
    !G_APP.s.selectedEntryId &&
      G_APP.s.selectedTreeLabel &&
      G_DOM.labelsInput instanceof HTMLInputElement &&
      (G_DOM.labelsInput.value = G_APP.s.selectedTreeLabel);
    if (!announce) {
      return;
    }
