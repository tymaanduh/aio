...
function setUniverseRenderMode(mode, options = {}) {
  ...
  const targetMode = mode === CONSTANT_UNIVERSE.VIEW_MODE_CANVAS ? CONSTANT_UNIVERSE.VIEW_MODE_CANVAS : CONSTANT_UNIVERSE.VIEW_MODE_WEBGL;
  ...
  announce && (setStatus(
      targetMode === CONSTANT_UNIVERSE.VIEW_MODE_WEBGL ? "Universe renderer set to WebGL." : "Universe renderer set to Canvas."
    ));
  return true;
}
...
function completeUniverseBenchmark(reason = "completed") {
  ...
  (baseCamera) && (G_UNI.view.zoom = clampNumber(baseCamera.zoom, CONSTANT_UNIVERSE.ZOOM_MIN, CONSTANT_UNIVERSE.ZOOM_MAX), G_UNI.view.panX = clampUniversePan(baseCamera.panX), G_UNI.view.panY = clampUniversePan(baseCamera.panY), clearProjectionCache());
  renderPerfHud(true);
