...
(function (root, factory) {
  ...
  function bindEvents() {
    ...
    [G_DOM.uiThemeEnterpriseInput, G_DOM.uiThemeFuturisticInput, G_DOM.uiThemeMonochromeInput].forEach((input) => {
    ...
    G_DOM.commandPaletteInput.addEventListener("keydown", (event) => {
      ...
      if (event.key === "ArrowUp") {
        ...
        G_RT.cmdIdx = (G_RT.cmdIdx - 1 + G_RT.cmdItems.length) % G_RT.cmdItems.length;
        renderCmdList();
        return;
      }
      if (event.key === "Enter") {
    ...
    bindActionElement(G_DOM.exportDataAction, () => {
      const format = G_DOM.exportFormatSelect instanceof HTMLSelectElement ? G_DOM.exportFormatSelect.value : "json";
    ...
    document.addEventListener("click", (event) => {
      const target = event.target;
      !G_DOM.contextMenu.classList.contains("hidden") &&
        (!(target instanceof Node) || !G_DOM.contextMenu.contains(target)) &&
        closeContextMenu();
    ...
    window.addEventListener("keydown", (event) => {
      ...
      (event.metaKey || event.ctrlKey) &&
        ...
        (event.preventDefault(), beginNewEntryInLabel(""));
    ...
    bindAutoCommitField(G_DOM.wordInput, {
      ...
      onBlur: () => {
        ...
        mode !== "code" &&
          mode !== "bytes" &&
          word.length >= MIN_LOOKUP_LENGTH &&
          (clearLookupTimer(), lookupAndSaveEntry(word));
      }
    });
